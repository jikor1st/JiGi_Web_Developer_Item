...
const diffMinutes = Math.ceil((date1- date2) / (1000 * 60)); // 분
```


```javascript
export const dateDiffCalc = (dateGet)=> {
    //등록일자 데이터 가공
    const nowDate = new Date();
    const thisDate = new Date(dateGet);
    let diffDay =  Math.ceil((nowDate.getTime() - thisDate.getTime())  / ( 1000 * 3600 * 24 ));
    let diffHours = Math.ceil((nowDate.getTime() - thisDate.getTime())  / ( 1000 * 3600 ));
    let diffMinutes = Math.ceil((nowDate.getTime() - thisDate.getTime())  / ( 1000 * 60 ));
    ...
    if(diffMinutes < 60){ // 1시간 이전 판단
        ...
        result = thisDate.getFullYear()+'-'+ ( (thisDate.getMonth() + 1) < 10 ? "0" + (thisDate.getMonth() + 1) : ( thisDate.getMonth() + 1 ) ) +'-'+ ( thisDate.getDay() < 10 ? "0" + thisDate.getDay() : thisDate.getDay() );
    }
